...
class PDFLoader:
    ...
    def _load_single_pdf(self, pdf_path: Path) -> list[Document]:
        ...
        for page_num in range(len(doc)):
            ...
            text = self.text_cleaner.clean_text(
                text, fix_spaces=True, fix_encoding=True, fix_math=True, normalize_whitespace=True
            )
        ...
        for img_index, img in enumerate(image_list):
            try:
                ...
                image_path = self._save_image(
                    image_bytes, book_title, page_num, img_index, image_ext
                )
    ...
    def _save_image(
        self, image_bytes: bytes, book_title: str, page_num: int, img_index: int, ext: str
    ) -> Path:
