...
class ExtractionQualityAnalyzer:
    ...
    def _get_basic_stats(self) -> Dict:
        ...
        return {
            ...
            'page_length_std': statistics.stdev([len(d['content']) for d in self.documents]) if total_pages > 1 else 0,
            'word_length_std': statistics.stdev([len(d['content'].split()) for d in self.documents]) if total_pages > 1 else 0,
        }
    ...
    def _analyze_math_content(self) -> Dict:
                    ...
                    if pattern_name == 'latex_commands':
                        for match in matches:
                            if any(cmd in match for cmd in ['frac', 'sqrt', 'sum', 'int']):
                                math_analysis['equation_extraction_quality']['complexity_distribution']['complex'] += 1
                            else:
                                math_analysis['equation_extraction_quality']['complexity_distribution']['simple'] += 1
        ...
        broken_patterns = [
            r'\$[^\$]*\$[^\$]*\$',  # Unclosed math
            r'\\\[.*?\\\]',  # Proper display math
            r'\\\(.*?\\\)',  # Proper inline math
        ]
            ...
            for element_name, pattern in self.structural_patterns.items():
                ...
                if matches:
                    ...
                    if element_name == 'section':
                        ...
                        structure['definitions_found'].extend(matches) if element_name == 'definition' else structure['theorems_found'].extend(matches)
            
            # Track section hierarchy depth
            if any(re.search(pattern, content, re.MULTILINE | re.IGNORECASE) for pattern in 
    ...
    def _analyze_per_book(self) -> Dict:
        ...
        for book, data in per_book.items():
            ...
            data['quality_metrics'] = {
                ...
                'expected_math_density': self.book_patterns.get(book, {}).get('expected_math_density', 0.3),
            }
    ...
    def _calculate_quality_score(self, results: Dict) -> int:
        ...
        encoding_issues = sum(len(pages) for pages in results['text_quality']['encoding_issues'].values())
    ...
    def _generate_chunking_recommendations(self, results: Dict) -> Dict:
        ...
        math_density = len(results['math_content']['pages_with_math']) / results['basic_stats']['total_pages']
        if math_density > 0.3:
        ...
        if len(results['page_quality']['short_pages']) > results['basic_stats']['total_pages'] * 0.3:
        ...
        recommendations['chunking_params'] = {
            ...
            'split_method': 'semantic' if 'semantic_chunking' in recommendations['strategies'] else 'fixed',
    ...
    def generate_report(self, results: Dict = None) -> str:
        ...
        if results['quality_score'] < 70:
            ...
            if results['text_quality']['garbled_pages']:
                report.append(f"   • {len(results['text_quality']['garbled_pages'])} pages have garbled text")
            if len(results['page_quality']['empty_pages']) > 10:
                report.append(f"   • {len(results['page_quality']['empty_pages'])} empty pages found")
            if results['math_content']['math_preservation_quality'] < 50:
                report.append(f"   • Poor math extraction quality ({results['math_content']['math_preservation_quality']:.1f}%)")
        ...
        report.append(f"   Pages with Math: {len(mc['pages_with_math'])} ({len(mc['pages_with_math'])/results['basic_stats']['total_pages']*100:.1f}%)")
        ...
        if mc['equation_extraction_quality']['complete_equations'] > 0:
            report.append(f"   Complete Equations Detected: {mc['equation_extraction_quality']['complete_equations']}")
        if mc['equation_extraction_quality']['broken_equations'] > 0:
            report.append(f"   ⚠️  Broken Equations: {mc['equation_extraction_quality']['broken_equations']}")
        ...
        for book, data in results['per_book_analysis'].items():
            ...
            report.append(f"      Math Pages: {len(set(data['math_pages']))} ({data['quality_metrics']['math_density']*100:.1f}%)")
            report.append(f"      Expected Math Density: {data['quality_metrics'].get('expected_math_density', 0.3)*100:.1f}%")
            report.append(f"      Extraction Grade: {data['quality_metrics'].get('math_extraction_grade', 'N/A')}")
            report.append(f"      Avg Words/Page: {data['quality_metrics']['avg_words_per_page']:.1f}")
    ...
    def export_detailed_metrics(self, results: Dict = None) -> Dict:
        ...
        flat_metrics = {
            ...
            'equations_found': results['math_content']['equation_extraction_quality']['complete_equations'],
            'broken_equations': results['math_content']['equation_extraction_quality']['broken_equations'],
